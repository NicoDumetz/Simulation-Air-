...
{
    game->background.texture = sfTexture_createFromFile("./sprite/map.jpg",
    NULL);
    game->background.sprite = sfSprite_create();
    game->background.pos = (sfVector2f){0, 0};
    game->background.scale = (sfVector2f){0.35, 0.35};
...
static void set_hitbox_tower(struct game *game, int j, int i, char **settings)
{
    ...
    hitbox.radius);
    game->tower[i].hitbox.pos = (sfVector2f){(game->tower[i].pos.x + (311
    * 0.4) / 2) - game->tower[i].hitbox.radius, (game->tower[i].pos.y +
    (311 * 0.4) / 2) - game->tower[i].hitbox.radius};
